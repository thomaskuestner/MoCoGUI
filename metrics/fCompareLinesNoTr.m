function [dice,dice0,dice1] = fCompareLinesNoTr(SRegiResult,Labels,hImg,names,iNGates)
% compare line segments in LandmarkGUI of transformed images
%
% input:
    ...
    if isempty(results.cLines.G01{xline(lineNo),yline(lineNo)}) || isempty(results.cLines.G02{xline(lineNo),yline(lineNo)})...
        ...
        for gateNo = 2:iNGates
            ...
            if strcmp(SCT,'sag')
                ...
                MRImage = squeeze(SRegiResult.dImg(:,sliceNo,:,1));
            ...
            % no transformation 'moving' line is registered line
            RegLine = MovLine;
            ...
            LineImage(:,:,3) = RegLine;
            LineImage = 6*LineImage + 4*MRImage;
